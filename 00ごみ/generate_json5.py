...
# 複数のAPIキーをリストで設定（429発生時に順番に切り替えます）
API_KEYS = [
...
# 処理したいYouTube URLのリスト（複数指定可能）
TARGET_URLS = [
    "https://www.youtube.com/watch?v=qjRQkRB9NOw",
"https://www.youtube.com/watch?v=mzhTUsXW30c",
"https://www.youtube.com/watch?v=rXhswIRVw-U",
]
...
def parse_and_fix_json(json_str: str):
    ...
    try:
        ...
        print(
            "⚠️ 途中で切れた不完全なJSONを検知しました。補正処理を実行します..."
        )
        ...
        if last_valid_object_index != -1:
            ...
            try:
                ...
                print(
                    "✅ 破損していた末尾をカットし、正常なJSONとして復元しました！"
                )
                return fixed_data
            except json.JSONDecodeError:
                pass
...
def get_video_metadata(video_id: str):
    ...
    with YoutubeDL(ydl_opts) as ydl:
        ...
        return original_title, formatted_date, auto_keywords
...
for url in TARGET_URLS:
    ...
    try:
        print("メタ情報を取得中...")
        original_title, published_at, auto_keywords = get_video_metadata(
            video_id
        )
        ...
        for item in fetched:
            ...
            transcript_list.append(
                {"start": start_time, "end": end_time, "text": item.text}
            )
...
"""
    ...
    # 4. Gemini API 呼び出し（429キー切替 ＆ 503リトライ処理付き）
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # --- 429 RESOURCE_EXHAUSTED キャッチ処理 ---
            if is_429:
                print(
                    f"⚠️ [429 RESOURCE_EXHAUSTED] クォータ上限・レートリミットを検知しました。"
                )
                ...
                if current_key_index + 1 < len(API_KEYS):
                    current_key_index += 1
                    print(
                        f"🔄 APIキーをIndex {current_key_index} に切り替えて再試行します..."
                    )
                    client = get_client(current_key_index)

                    # キーを更新して同じ attempt 内で即時リトライ（attempt カウントを増やさない調整）
                    try:
                        response = client.models.generate_content(
                            model="gemini-3.5-flash",
                            contents=prompt,
                        ...
                        print(
                            f"❌ 新しいキーでもエラーが発生しました: {retry_e}"
                        )

                else:
                    print(
                        "⚠️ 使用できる代替APIキーがこれ以上ありません。"
                    )

            # --- 503 サーバーエラーまたはその他リトライ処理 ---
            if is_503 or is_429:
                print(f"⚠️ ({attempt}/{max_retries}回目の試行失敗)")
                if attempt < max_retries:
                    print("5秒後に再試行します...")
                    time.sleep(5)
                else:
                    print(
                        "❌ 再試行回数が上限に達しました。プログラムを異常終了します。"
                    )
                    sys.exit(1)
            else:
    ...
    gemini_members = [
        f"#{m.replace('❓', '')}" for m in json_data.get("members", [])
    ]
    ...
    video_entry = {
        "id": video_id,
        "title": json_data.get("title", original_title),
    ...
    for item in videos_list:
        if item.get("id") == video_id:
            ...
            merged_raw = list(
                dict.fromkeys(item.get("keywords", []) + filtered_keywords)
            )
            item["keywords"] = [
                tag for tag in merged_raw if tag in ALLOWED_TAGS
            ]
            is_updated = True
            break
    ...
    print(
        f"✅ 『{videos_file}』を更新完了しました！（ID: {video_id}）"
    )
